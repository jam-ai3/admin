...
import { getRecentUsers, getUsageData } from "./_actions/db-functions";
...
function formatUsers(calls: User[]): FormattedUsersProps[] {
  ...
  const chartReadyData: FormattedUsersProps[] = Object.entries(formattedUsers).map(
    ([key, totalUsers]) => ({
...
function getUserCardInfo(users: User[], numDays: number, description: string): SectionTypeProps {
  ...
  const filteredTwoDaysAgo = users.filter(user => user.createdAt > twoDaysAgo && user.createdAt < oneDayAgo)
  const percentage = (filteredOneDayAgo.length === 0 ? 0 : ((filteredOneDayAgo.length - filteredTwoDaysAgo.length) / filteredTwoDaysAgo.length) * 100)
...
function getCallsCardInfo(calls: IndividualCall[], numDays: number, description: string): SectionTypeProps {
  const oneDayAgo = new Date(Date.now() - numDays * 24 * 60 * 60 * 1000)
  const twoDaysAgo = new Date(Date.now() - numDays * 2 * 24 * 60 * 60 * 1000)
  const filteredOneDayAgo = calls.filter(call => call.createdAt > oneDayAgo)
  const filteredTwoDaysAgo = calls.filter(call => call.createdAt > twoDaysAgo && call.createdAt < oneDayAgo)
  const percentage = (filteredOneDayAgo.length === 0 ? 0 : ((filteredOneDayAgo.length - filteredTwoDaysAgo.length) / filteredTwoDaysAgo.length) * 100)
  const uptread = percentage >= 0
...
export default async function Home() {
  ...
  const userDailyCardInfo = getUserCardInfo(users, 1, "User Sign ups in the Last Day")
  const userWeeklyCardInfo = getUserCardInfo(users, 7, "User Sign ups in the Last Week")
  const callsDailyCardInfo = getCallsCardInfo(calls, 1, "Number of Calls in the Last Day")
  const callsWeeklyCardInfo = getCallsCardInfo(calls, 7, "Number of Calls in the Last Week")
